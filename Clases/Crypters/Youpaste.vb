Namespace Crypters
    ...
    Public Class YouPaste
        ...
        Public Shared Function IsYouPaste(FileID As String) As Boolean
            If String.IsNullOrEmpty(FileID) Then Return False
            Return FileID.ToLower.StartsWith(URLExtractor.YOUPASTETOKEN.ToLower) And FileID.Split("$"c).Length = 3
        End Function
        ...
        Public Shared Function ObtenerInformacionFichero(ByVal Config As Configuracion, ByVal FileID As String, ByVal FileKey As String, ByVal ComprobacionAntesDescarga As Boolean) As Conexion.InformacionFichero
            If Not IsYouPaste(FileID) Then Throw New ApplicationException("Error, not YouPaste link")
            Dim LinkYouPaste As String = "!" & FileID.Split("$"c)(1) & "!" & FileID.Split("$"c)(2)
            ...
            For Each Peticion As String In Peticiones
                Dim json As String = String.Format("{{""link"":""{0}"",""m"":""{1}""}}", LinkYouPaste, Peticion)

                Dim Resultado As Conexion.Respuesta = Conexion.SendJSON("http://youpaste.co/api", json, SendAppId:=False)
                ...
                If Resultado.Excepcion Is Nothing Then
                    ...
                    Try
                        ...
                        If IsNumeric(str) Then
                            Throw New ApplicationException("Invalid YouPaste link (" & str & ")")
                        End If
                        ...
                        If FileInfoRS.ContainsKey("error") Then
                            Throw New ApplicationException("Invalid YouPaste link (" & CStr(FileInfoRS.Item("error")) & ")")
                        End If

                    Catch excFileInfo As Exception
                        Log.WriteError("Error getting the info in YouPaste - Error: " & excFileInfo.ToString)
                        Info.Err = Conexion.TipoError.Otros
                        Info.Errtxt = "Error getting the info in YouPaste: " & excFileInfo.Message
                        Return Info
                    End Try
                ...
                ElseIf TypeOf (Resultado.Excepcion) Is System.Net.WebException Then
                    ...
                    Log.WriteError("Error getting the info in YouPaste: " & Resultado.Excepcion.Message & " - Message received: " & Resultado.Mensaje)
                    Return Info
                Else
                    Info.Err = Conexion.TipoError.Otros
