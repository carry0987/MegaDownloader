Namespace Crypters
    ...
    Public Class MegaCrypter

        Public Shared Function IsMegaCrypter(FileID As String) As Boolean
            If String.IsNullOrEmpty(FileID) Then Return False
            Return FileID.ToLower.StartsWith(URLExtractor.MEGACRYPTERTOKEN.ToLower) And FileID.Split("$"c).Length = 3
        End Function
        ...
        Public Shared Function ObtenerInformacionFichero(ByVal Config As Configuracion, ByVal FileID As String, ByVal FileKey As String, ByVal ComprobacionAntesDescarga As Boolean) As Conexion.InformacionFichero
            If Not IsMegaCrypter(FileID) Then Throw New ApplicationException("Error, not MegaCrypter link")
            ...
            For Each Peticion As String In Peticiones
                ...
                Dim Resultado As Conexion.Respuesta = Conexion.SendJSON("http://megacrypter.com/api", json, SendAppId:=False)
                ...
                If Resultado.Excepcion Is Nothing Then
                    ...
                    Try
                        ...
                        If IsNumeric(str) Then
                            Throw New ApplicationException("Invalid Megacrypter link (" & str & ")")
                        End If
                        ...
                        If FileInfoRS.ContainsKey("error") Then
                            Throw New ApplicationException("Invalid Megacrypter link (" & CStr(FileInfoRS.Item("error")) & ")")
                        End If

                    Catch excFileInfo As Exception
                        Log.WriteError("Error getting the info in MegaCrypter - Error: " & excFileInfo.ToString)
                        Info.Err = Conexion.TipoError.Otros
                        Info.Errtxt = "Error getting the info in MegaCrypter: " & excFileInfo.Message
                        Return Info
                    End Try
                ...
                ElseIf TypeOf (Resultado.Excepcion) Is System.Net.WebException Then
                    ...
                    Log.WriteError("Error getting the info in MegaCrypter: " & Resultado.Excepcion.Message & " - Message received: " & Resultado.Mensaje)
                    Return Info
                Else
                    Info.Err = Conexion.TipoError.Otros
